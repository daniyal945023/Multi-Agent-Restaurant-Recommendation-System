...
"""
    ...
    try:
        ...
        user_profile = json.loads(response) #convert json response to python dict
        print(f"✓ User profile generated: {user_profile.get('summary', 'No summary')}")
    except Exception as e:
        print(f"⚠ Error generating profile: {e}")
        user_profile = {"error": str(e)}
    
    state["user_profile"] = user_profile #store the user profile dict in user_profile in state
    state["workflow_step"] = "profile_generated" #track the current step of the workflow inside the state
    return state
...
"""
    ...
    #retrieve restaurants and recipes from vector db, based on user_profile and store them in the state
...
def node_analyze_styles(state: dict) -> dict:
    ...
    user_message = f"""Analyze the food styles and flavor profiles based on this data:
      "Restaurants: {json.dumps(restaurants[:5],indent=2)},
      Recipes: {json.dumps(recipes[:5], indent=2)},
...
Identify 3-5 relevant trends and explain how they align with modern dining culture.
Return JSON: {{
  "dominant_flavors": ["flavor_profile_1", "flavor_profile_2"],
  "cuisine_compatibility": [
    {{
      "cuisine": "Cuisine Name",
      "compatibility_score": 95,
      "reasoning": "Why this culinary style fits the user's profile."
    }}
  ],
  "preferred_textures": ["texture_1", "texture_2"],
  "culinary_style_summary": "A brief overview of how the recommended styles match the user's culinary lifestyle."
}}
    """  
...
def run_workflow(user_input: str) -> dict:
    ...
    # Phase 1 – Sequential
    state = node_generate_profile(state) #generate user profile based on user data and save it in state
    
    # Phase 2 – Sequential
    state = node_retrieve_candidates(state) #based on user profile, get restaurants and recipes from vector db and save them in state
    ...
    #Do all 3 concurrently: analyze trends,analyze styles and evaluate nutrition, all at same time, instead of in sequence
