...
def fuse_rank(
    ...
    return rows[:top_n] #return only top_n rows(based on fusion score) as rows r sorted

def print_fused(rows, title: str, max_chars: int = 90):
    print(f"\n=== {title} ===")  #print the data  of the top n fusion score retrievals
...
text_heavy_rows = fuse_rank(q,5,5,0.8,0.2,where_articles,where_images,5)
print_fused(text_heavy_rows, title="Demo 3A — Text-heavy fusion (w_text=0.8,w_img=0.2)")
...
img_heavy_rows = fuse_rank(q,5,5,0.3,0.7,where_articles,where_images,5)
print_fused(img_heavy_rows, title="Demo 3B — image-heavy fusion (w_text=0.3,w_img=0.7)")
